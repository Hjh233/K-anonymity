...
def LossMetric(hierarchy, SuppressionNumber):
    ...
    LossTotal = LossSex + LossRace + LossMarital + LossAge + SuppressionNumber * 4 /30162
...
def Samarati(data,Lattice): # (data,Lattice,LMRV)
    low = 0
    high = 8
    # ReleaseNum = 0
    while (low < high):
...
# Refined Samarati with smaller loss metric

LMHV = []   # Loss Metric Hierarchy Vector
HierarchyVector = []
    ...
    while (low < high):
        ...
        for VECTOR in Lattice[mid]:
            ...
            if Releasable(Cluster(GTC)) == 0:
                ...
                ReleaseVector = copy.deepcopy(VECTOR)
                HV.append(ReleaseVector)
                SupNum = Cluster(data)
                LM.append(LossMetric(ReleaseVector, SupNum))
                high = mid

    Min = min(LM)
    MinIndex = LM.index(Min)

    ReleaseVector = HV[MinIndex]
...
VectorCandidate = Samarati(data, Lattice, LMHV, HierarchyVector)
print('\n')
...
print(GT)
print('Release Table is shown as below: ')
print(GT[(QI + SV)])
...
LossVC = LossMetric(VectorCandidate,LocSup)
